	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		else
		{
			...
			int t1=service.deleteSubject(Integer.parseInt(request.getParameter("subject_id")));  
			  if(t1==1) {
				  out.println("Subject Not Found!! Having ID number:"+request.getParameter("subject_id"));
				  rd.include(request, response);
			  }
			  out.println("Subject Deleted Successfully! Having ID number:"+request.getParameter("subject_id"));
			  rd.include(request, response);
		}
	}
}
